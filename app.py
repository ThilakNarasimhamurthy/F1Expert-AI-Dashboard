...
def generate_f1_trivia():
    trivia_prompt = """
    Generate 5 unique and interesting Formula 1 trivia questions with their answers.
    Format your response as follows (include the JSON structure):
    ```json
    [
        {
    ...
    ```
    Make sure each question is challenging but fair, focusing on F1 history, records, and interesting facts.
    """
    try:
        ...
        # Clean up the string in case there are issues
        json_str = json_str.replace('\\n', '').replace('\\', '')
        questions = json.loads(json_str)
        return questions
    except Exception as e:
        st.error(f"Error generating trivia: {str(e)}")
        return [{"question": "What team has won the most constructor championships in F1 history?",
                 "answer": "Ferrari has won the most constructor championships."}]
        ...
        if st.session_state.trivia_questions:
            for i, qa in enumerate(st.session_state.trivia_questions):
                with st.container():
                    ...
                    # Only show input if answer isn't revealed yet
                    if not st.session_state.show_answers[i]:
                        ...
                        with col2:
                            if st.button("Submit", key=f"submit_{i}"):
                                ...
                                # Simple answer checking (case-insensitive substring matching)
                                correct_answer = qa['answer'].lower()
                                user_input = user_answer.lower()

                                # Check if user's answer contains key words from the correct answer
                                key_terms = correct_answer.split()
                                significant_terms = [term for term in key_terms if len(term) > 3]  # Only check significant words

                                # Check if any significant terms are in the user's answer
